...
function displayEmployeeCompetencyReport() {
    ...
    if (!displayArea) { console.error("[displayEmployeeCompetencyReport] Report display area not found!"); return; }
    ...
    filteredEmpData = filteredEmpData.filter(emp => emp.assessedCompetencies && emp.assessedCompetencies.length > 0);
    ...
    configurableCompetencyCategories.forEach(cat => {
        (cat.types || []).forEach(type => {
            typesMap.set(type.id, { 
                ...
                levelDescriptions: type.levelDescriptions || [] // Ensure this exists
            });
        });
    });

    let reportHTML = `<h3 style="margin-bottom:15px;">Employee Competency Report</h3>`;
    reportHTML += '<div class="competency-report-cards-container" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 20px;">';
        ...
        if (emp.assessedCompetencies.length > 0) {
            ...
            emp.assessedCompetencies.forEach(ac => {
                ...
                reportHTML += `<li style="margin-bottom:10px; padding-bottom:8px; border-bottom: 1px dotted #eee;">
                                ...
                                ${ac.evidence ? `Evidence: <span style="font-style:italic; color:#555; display:block; max-height: 40px; overflow:hidden; text-overflow:ellipsis;">${ac.evidence.replace(/\n/g, '<br>')}</span>` : ''}
                              </li>`;
            });
            reportHTML += '</ul>';
        } else {
    ...
    console.log("[displayEmployeeCompetencyReport] Report HTML rendered.");
}
...
function displayRolePerformanceComparison(selectedRoleName) {
    ...
    // ---- ADDED LOGS AND BUTTONS ----
    console.log("[DEBUG] displayRolePerformanceComparison: Just before checking recordsToCompare.length for action buttons.");
    console.log("[DEBUG] displayRolePerformanceComparison: recordsToCompare contents:", JSON.parse(JSON.stringify(recordsToCompare)));
    console.log("[DEBUG] displayRolePerformanceComparison: recordsToCompare.length IS:", recordsToCompare.length);

    if (recordsToCompare.length > 0) {
        console.log("[DEBUG] displayRolePerformanceComparison: Condition (recordsToCompare.length > 0) is TRUE. Adding action buttons.");
        reportHTML += `
            <div id="report-actions-role-comparison" class="report-actions-container" style="margin-top: 25px; padding-top: 15px; border-top: 1px solid #ccc;">
                <button id="download-csv-role-comparison" class="report-action-button">Download CSV</button>
                <button id="download-excel-role-comparison" class="report-action-button" style="margin-left: 10px;">Download Excel (as CSV)</button>
                <button id="print-report-role-comparison" class="report-action-button" style="margin-left: 10px;">Print Report</button>
            </div>
        `;
    } else {
        console.log("[DEBUG] displayRolePerformanceComparison: Condition (recordsToCompare.length > 0) is FALSE. NOT adding action buttons.");
    }
    // ---- END ADDED LOGS AND BUTTONS ----
    displayArea.innerHTML = reportHTML;
    ...
    document.querySelectorAll('.view-compare-kpi-details-button').forEach(button => {
        button.addEventListener('click', (event) => {
            ...
            if (detailsDiv && specificRecord && specificRecord.kpiDetails) {
                ...
                if (detailsDiv) detailsDiv.innerHTML = '<p>Could not load KPI details for this record.</p>';
                if (btn) { btn.textContent = 'Details Error'; btn.disabled = true; }
            }
        });
    });

    const downloadCSVRoleBtn = document.getElementById('download-csv-role-comparison');
    if (downloadCSVRoleBtn) {
        downloadCSVRoleBtn.addEventListener('click', () => {
            const filename = `Role_Comparison_${selectedRoleName.replace(/\s+/g, '_')}_${comparisonPeriodForDisplay.replace(/\s*\(Latest\)\s*/, '').replace(/[^a-zA-Z0-9_-]/g, '-')}.csv`;
            const csvData = generateRoleComparisonReportCSV(recordsToCompare, selectedRoleName, comparisonPeriodForDisplay);
            if (csvData) triggerCSVDownload(csvData, filename);
            else alert('No data to download for Role Comparison.');
        });
    }
    const downloadExcelRoleBtn = document.getElementById('download-excel-role-comparison');
    if (downloadExcelRoleBtn) {
        downloadExcelRoleBtn.addEventListener('click', () => {
            const filename = `Role_Comparison_${selectedRoleName.replace(/\s+/g, '_')}_${comparisonPeriodForDisplay.replace(/\s*\(Latest\)\s*/, '').replace(/[^a-zA-Z0-9_-]/g, '-')}.csv`;
            const csvData = generateRoleComparisonReportCSV(recordsToCompare, selectedRoleName, comparisonPeriodForDisplay);
            if (csvData) triggerCSVDownload(csvData, filename);
            else alert('No data to download for Role Comparison.');
    ...
    const printReportRoleBtn = document.getElementById('print-report-role-comparison');
    if (printReportRoleBtn) {
        printReportRoleBtn.addEventListener('click', () => window.print());
    }
    console.log("[displayRolePerformanceComparison] Report HTML (including action buttons if any) rendered.");
}
